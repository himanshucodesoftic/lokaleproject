...
class OrdersController extends Controller
{
    ...
    {   
        ...
        if (count($result['cart']) == 0) {
            ...
            if (Session::get('guest_checkout') == 1 && session('shipping_address')) {                
                ...
                $customers_id = auth()->guard('customer')->user() ? auth()->guard('customer')->user() ->id : "";
                $result['customer'] = auth()->guard('customer')->user() ? auth()->guard('customer')->user() : null;
            }
    ...
    public function upay(Request $request) {
        ...
        $refernce = 'Ref'.time();
        $extraMerchantData = '{"amounts":['.$request->total_price.'],"charges":["0.5"],"chargeType":["fixed"],"cc_charges":["3.2"],"cc_chargeType":["percentage"],"ibans":["KW48BBYN0000000000000394689002"]}';
        
        if ($request->upay_environment == "Test") {
            $fields = array(
                'merchant_id'=>$request->upay_merchant_id,
                'username' => $request->upay_username,
                'password'=>stripslashes($request->upay_password),
                'api_key'=>$request->upay_public_key, // in sandbox request
                'order_id'=> $orderID, // MIN 30 characters with strong unique function (like hashing function with time)
                'total_price'=> (float)$request->total_price,
                'CstFName'=>$request->customer_name,
                'CstEmail'=>$request->customer_email,
                'CstMobile'=>$request->customer_phone,
                'success_url'=> $websiteURL.'upaysuccess',
                'error_url'=> $websiteURL.'upayerror',
                'test_mode'=>1, // test mode enabled
                'reference'=> $refernce, // Reference that you want to show in invoice in ref column
                'ExtraMerchantsData'=> $extraMerchantData
            );
        } else {
            $fields = array(
                'merchant_id'=>$request->upay_merchant_id,
                'username' => $request->upay_username,
                'password'=>stripslashes($request->upay_password),                
                'api_key' =>password_hash($request->upay_public_key,PASSWORD_BCRYPT), //In production mode, please pass API_KEY with BCRYPT function
                'order_id'=> $orderID, // MIN 30 characters with strong unique function (like hashing function with time)
                ...
                'ProductTitle' => 'Upayment Order',
                'ProductName'=>stripslashes($request->product_names),
                'ProductQty'=>$request->product_qtys,
        ...
        if ($request->upay_environment == "Test") {
            $pay_url = "https://api.upayments.com/test-payment";
        } else {
            $pay_url = "https://api.upayments.com/payment-request";
        }        
        ...
        if ($server_output['status'] == 'success') {
            ...
            Session::forget('paymentResponseData');

            session(['upay_transaction_id' => $orderID]);
            session(['upay_json' => array(
                'paid' => 'true',
                'order_id' => $orderID,
                'reference' => $refernce
            )]);
            ...
            session(['paymentResponseData' => 'UPayment process error']);
            return redirect()->back()->with('error', "Error occured");
        }
    }

    public function upaysuccess(Request $request) {
        session(['upayr' => 'success']);
        return redirect('/checkout');
    }

    public function upayerror(Request $request) {
        Session::forget('upay_transaction_id');
        Session::forget('upay_json');
        session(['paymentResponse' => 'error']);
        session(['paymentResponseData' => 'UPayment process error']);
        return redirect('/checkout');
    }
    ...
    public function checkout_shipping_address(Request $request)
    {
        ...
        $billing_data['billing_company'] = 'LokalKW';
    ...
    public function shipping_methods()
    {
        ...
        if (!empty(session('shipping_address'))) {
            $countries_id = session('shipping_address')->countries_id;
            $toPostalCode = '12345';//session('shipping_address')->postcode;
    ...
    public function getPaymentMethods()
    {
        ...
        $upayments_setting = $this->order->payments_setting_for_upay();  

        if ($upayments_setting['merchant_id']->environment == '1') {
            $enviroment = 'Test';
        } else {
            $enviroment = 'Live';
        }
        ...
        $upays = array(
            'environment' => $enviroment,
            'payment_currency' => Session::get('currency_code'),
            'public_key' => $upayments_setting['api_key']->value,
            'merchant_id' => $upayments_setting['merchant_id']->value,
            'name' => $upayments_setting['merchant_id']->name,
            'username' => $upayments_setting['username']->value,
            'password' => $upayments_setting['password']->value,
            'active' => $upayments_setting['merchant_id']->status,
            'payment_method' => $upayments_setting['merchant_id']->payment_method,            
        );
