...
class MediaController extends Controller
{
    ...
    public function add()
    {
        ...
        return view('web.media.addimages')->with('images', $images)->with('result', $result);
    }
    ...
    public function fileUpload(Request $request)
    {
        ...
        if ($request->hasFile('file') and in_array($request->file->extension(), $extensions)) {
            ...
            if (!empty(session('images'))) {
                $u_images = session('images');
            }

            array_push($u_images, array(
                'id' => $imagedata,
                'path' => $Path
            ));
            
            session(['images' => $u_images]);
...
//        $upload_success = $thumbnailImage->save($directory, $filename, 'public');
    ...
    public function detailimage($id)
    {
        ...
        $result['commonContent'] = $this->Setting->commonContent();
        $returnHTML = view('web.media.detail')->with('result', $result);
