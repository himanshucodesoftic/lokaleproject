...
class AlertController extends Controller
{
	...
	////  new vendor alert
	public function vendorRequestAlert($requestData) {
		//alert setting
		$alertSetting = $this->getAlertSetting();
		//setting
		$setting = $this->setting();

		$requestData['app_name'] = $setting[18]->value;
		$requestData['admin_email'] = $setting[70]->value;

		if(!empty($requestData['admin_email'])){
			Mail::send('/mail/requestVendor', ['requestData' => $requestData], function($m) use ($requestData){
				$m->from('admin@gmail.com', $requestData['app_name']);
				$m->to($requestData['admin_email'])->subject(Lang::get("labels.NewVendorRequest"))->getSwiftMessage()
				->getHeaders()
				->addTextHeader('x-mailgun-native-send', 'true');
			});
		}
	...
	/////   New Car Email
	public function newCarRequest($requestData) {
		//alert setting
		$alertSetting = $this->getAlertSetting();
		//setting
		$setting = $this->setting();
		...
		if(!empty($requestData['admin_email'])){
			Mail::send('/mail/newCarRequest', ['requestData' => $requestData], function($m) use ($requestData){
				$m->from('admin@gmail.com', $requestData['app_name']);
				$m->to($requestData['admin_email'])->subject(Lang::get("website.NewProductRequest"))->getSwiftMessage()
				->getHeaders()
				->addTextHeader('x-mailgun-native-send', 'true');
			});
		}
