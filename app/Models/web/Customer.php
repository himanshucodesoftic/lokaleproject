...
class Customer extends Model
{
    ...
    public function processLogin($request, $old_session)
    {
        ...
        //set session
        session(['customers_id' => $customer->id]);
        session(['role_id' => $customer->role_id]);
    ...
    public function checkVendor($request) {
        $result = array();
        $vendorsByEmail = DB::table('vendors')->where('email', $request->email)->get();
        $vendorsByTitle = DB::table('vendors')->where('vendor_name', $request->vendor_name)->get();
        $usersByEmail = DB::table('users')->where('email', $request->email)->get();

        $requestsByEmail = DB::table('vendor_requests')->where('email', $request->email)->get();
        $requestsByTitle = DB::table('vendor_requests')->where('title', $request->vendor_name)->get();

        if (count($vendorsByEmail) > 0 || count($usersByEmail) > 0) {
            $result['email'] = 'email already exist';
        } else if (count($vendorsByTitle) > 0) {
            $result['title'] = 'title already exist';
        } else if (count($requestsByEmail) > 0) {
            $result['request_email'] = 'email already requested';
        } else if (count($requestsByTitle) > 0) {
            $result['request_title'] = 'title already requested';
        }
        return $result;
    }

    public function requestVendor($request) {
        $request_id = DB::table('vendor_requests')->insertGetId([
            'first_name' => $request->firstName,
            'last_name' => $request->lastName,
            'address' => $request->vendor_address,
            'city' => $request->vendor_city,
            'zipcode' => $request->vendor_zipcode,
            'country_id' => $request->country,
            'phone' => $request->phone_number,
            'title' => $request->vendor_name,
            'title_arabic' => $request->vendor_arabic_name,
            'email' => $request->email,
        ...
        //notification/email
        $requestData['vendor_title'] = $request->vendor_name;
        $requestData['customer_name'] = $request->firstName. ' ' .$request->lastName;
        $requestData['request_email'] = $request->email;
        $requestData['phone'] = $request->phone_number;
        $requestData['address'] = $request->vendor_address. ', ' .$request->vendor_city. ', ' .$request->vendor_zipcode;

        $myVar = new AlertController();
        $alertSetting = $myVar->vendorRequestAlert($requestData);
