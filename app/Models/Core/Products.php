...
class Products extends Model
{
	...
	public function paginator($request){
        ...
        $data->select('products.*', 'products_description.*', 'specials.specials_id',
        ...
        'specials.expires_date', 'image_categories.path as path', 'products.updated_at as productupdate', 'categories_description.categories_id', 'products.created_at as productcreate',
  ...
  public function edit($request){
    ...
    $product = DB::table('products')
        ...
        ->select('products.*','image_categories.*','products.updated_at as updateddate')
    ...
    if(!empty($product[0]->vendor_id)) {
        $vendor_data = DB::table('vendors')->leftJoin('users','users.id','=','vendors.vendor_id')
        ->where('vendors.vendor_id',$product[0]->vendor_id)
        ->select('vendors.vendor_name','vendors.email as vendor_email','vendors.phone as vendor_phone','users.first_name','users.last_name','users.email as owner_email','users.phone as owner_phone')->get();
  ...
  public function updaterecord($request){
        ...
        //     $user_email = DB::table('users')->where('id', $request->vendor_id)->first()->email;
        //     $statusData['price'] = $request->products_price;
        //     $statusData['request_email'] = $user_email;
        //     $statusData['car_title'] = $request->products_name_1;
        //     if ($request->products_status == 1) {
        //         $statusData['message'] = Lang::get("labels.requestAccepted");
        //     } else if ($request->products_status == 0) {
        //         $statusData['message'] = Lang::get("labels.requestRejected");
        //     } else {
        //         $statusData['message'] = Lang::get("labels.inreview");
        //     }
  ...
  public function deleterecord($request){
    ...
    if (!empty($prod_data[0]->vendor_id)) {
        $vendor_info = DB::table('users')->where('id', $prod_data[0]->vendor_id)->get();
        if (count($vendor_info) > 0) {
            $user_email = $vendor_info[0]->email;
        } else {
            $user_email = "support@lokalkw.com";
        }

        $statusData['price'] = $prod_data[0]->products_price;
        $statusData['request_email'] = $user_email;
        $statusData['car_title'] = $prod_data[0]->products_name;
        
        $statusData['message'] = Lang::get("labels.DeletedProject");
  ...
  public function publish($request){
    ...
    $prods = DB::table('products')->where('products_id', $products_id)->update([
        'products_status' => 1,
        'percentage' => $request->percentage
    ]);
    ...
    $user_email = DB::table('users')->where('id', $prod_data[0]->vendor_id)->first()->email;
  ...
  public function acceptchange($request) {
    $products_id      =   $request->pro_id;
    $prod_data = DB::table('products')
    ->leftJoin('products_description', 'products_description.products_id', '=', 'products.products_id')
    ->select('products.*','products_description.products_name')
    ...
    $images = [];
    $updated_images = json_decode($prod_data[0]->updated_images);
    $count = 0;
    if (count($updated_images) > 0) {
        if (count($updated_images) == 1) {
            DB::table('products')->where('products_id',$products_id)->update([
                'products_image' => $updated_images[0]->id
            ]);
        } else {
            DB::table('products_images')->where('products_id',$products_id)->delete();
            foreach($updated_images as $image) {
                if ($count == 0) {
                    DB::table('products')->where('products_id',$products_id)->update([
                        'products_image' => $image->id
                    ]);
                } else {
                    DB::table('products_images')->insert([
                        'products_id' => $products_id,
    ...
    $prods = DB::table('products')->where('products_id', $products_id)->update([
        ...
        'products_quantity' => $prod_data[0]->updated_quantity
    ]);


    $user_email = DB::table('users')->where('id', $prod_data[0]->vendor_id)->first()->email;

    $statusData['price'] = $prod_data[0]->updated_quantity;
    $statusData['request_email'] = $user_email;
    $statusData['car_title'] = $prod_data[0]->products_name;
  ...
  public function declinechange($request) {
    $products_id      =   $request->pro_id;
    $prod_data = DB::table('products')
    ->leftJoin('products_description', 'products_description.products_id', '=', 'products.products_id')
    ->select('products.*','products_description.products_name')
