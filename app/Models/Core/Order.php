    ...
    public function paginator($request = ''){
        ...
        if(!empty($request)) {
            ...
            if(!empty($request->vendor_id)) {
                session(['filter_vendor_id' => $request->vendor_id]);
                $products_ids = [];             
                foreach(DB::table('products')->where('vendor_id',$request->vendor_id)->select('products_id')->get() as $ppro) {
                    $products_ids[] = $ppro->products_id;
                }
                $order_ids = [];
                foreach(DB::table('orders_products')->whereIn('products_id',$products_ids)->select('orders_id')->get() as $ord_ids) {
                    $order_ids[] = $ord_ids->orders_id;
                }
                $query = DB::table('orders')->whereIn('orders_id',$order_ids);
            } else {
                Session::forget('filter_vendor_id');
            ...
            if(!empty($request->from_date) && !empty($request->to_date)) {
                ...
                $query->whereBetween('date_purchased',[$request->from_date.' 00:00:00',$request->to_date.' 23:30:00']);
            } else {
        ...
        $orders = $query->orderBy('date_purchased', 'DESC')
        ...
        foreach ($orders as $orders_data) {
            $final_price = DB::table('orders_products')->sum('final_price');

            $order_products = DB::table('orders_products')->where('orders_id',$orders_data->orders_id)->get();
            $orders[$index]->order_products = $order_products;

            $orders[$index]->total_price = $final_price;
