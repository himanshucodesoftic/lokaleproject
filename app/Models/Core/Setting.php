...
class Setting extends Model
{
    ...
    public function commonContent()
    {
        ...
        $new_vendors = DB::table('vendor_requests')->where('status', 2)->get();
        $result['new_vendor_requests'] = count($new_vendors);

        $new_pro_req = DB::table('products')->where('products_status', 2)->orWhere('update_request_status', 2)->get();
		$result['new_pro_req'] = count($new_pro_req);
